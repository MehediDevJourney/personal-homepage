/* =========================================
   1. ORIGINAL LOGOS (Categorized perfectly)
   ========================================= */
const apps = [
...
/* =========================================
   4. ANIMATED TYPEWRITER PLACEHOLDER
   ========================================= */
const searchInputBox = document.getElementById('searchInput');
const placeholders = ["Search the web...", "Ask ChatGPT...", "Go to a URL...", "Translate a word..."];
let pIdx = 0; let cIdx = 0; let isDel = false;

function typePlaceholder() {
    // Pause animation if user clicks inside the search box
    if(document.activeElement === searchInputBox) {
        searchInputBox.setAttribute('placeholder', 'Type here...');
    ...
    if (isDel) {
        searchInputBox.setAttribute('placeholder', currentText.substring(0, cIdx - 1) + '|');
        cIdx--;
    } else {
        searchInputBox.setAttribute('placeholder', currentText.substring(0, cIdx + 1) + '|');
        cIdx++;
    ...
    if (!isDel && cIdx === currentText.length) { speed = 2000; isDel = true; } 
    else if (isDel && cIdx === 0) { isDel = false; pIdx = (pIdx + 1) % placeholders.length; speed = 500; }
...
/* =========================================
   5. REAL SEARCH LOGIC & KEYBOARD NAV
   ========================================= */
...
searchInputBox.addEventListener('input', () => {
    clearBtn.style.display = searchInputBox.value.length > 0 ? 'flex' : 'none';
    clearTimeout(debounceTimer);
    
    const val = searchInputBox.value.trim();
    if(isURL(val)) {
        suggestions.innerHTML = `<li onclick="searchGo('${val}')"><svg viewBox="0 0 24 24"><path d="M21.71,20.29,18,16.61A9,9,0,1,0,16.61,18l3.68,3.68a1,1,0,0,0,1.42,0A1,1,0,0,0,21.71,20.29ZM11,18a7,7,0,1,1,7-7A7,7,0,0,1,11,18Z"/></svg><span style="color:var(--accent); font-weight:700;">Go directly to ${val}</span></li>`;
...
clearBtn.addEventListener('click', () => { 
    searchInputBox.value = ''; clearBtn.style.display = 'none'; suggestions.style.display = 'none'; searchInputBox.focus(); 
});
...
searchInputBox.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'ArrowDown') { currentFocus++; addActive(x); } 
        ...
        else { let val = searchInputBox.value.trim(); if (val) { searchGo(val); } } 
    }
});
...
document.addEventListener('mousedown', (e) => { if (!searchInputBox.contains(e.target) && !suggestions.contains(e.target) && e.target !== clearBtn) { suggestions.style.display = 'none'; searchInputBox.blur(); } });

/* =========================================
   6. SHORT & PUNCHY MOTIVATIONAL QUOTES
   ========================================= */
const quotes = [
